<script context="module">
    export async function preload({ params, query }) {
      ...
      const res = await this.fetch(`places/${params.slug}.json`);
  ...
  <!-- 	<meta name="twitter:creator" value="" /> -->
      ...
      <meta name="twitter:title" value={place.title} />
      <meta name="twitter:description" content={place.excerpt} />
  ...
  <!-- 	<meta name="twitter:image" content="" /> -->
      ...
      <meta
          name="twitter:data1"
          value={new Date(place.printDate).toLocaleDateString(undefined, {
              year: 'numeric',
              month: 'short',
              day: 'numeric'
          })} />
      <meta name="twitter:label2" value="Reading Time" />
      <meta name="twitter:data2" value={place.printReadingTime} />
  </svelte:head>
  
  <header>
    <p>{place.printDate} ~ {place.printReadingTime}</p>
    <h1>{place.title}</h1>
    <hr />
  </header>
  <div class="container">
